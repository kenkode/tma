...
<div class="pro-head">
      ...
      <a data-toggle="modal" id="add" class="btn btn-primary" href="#modal-form">Add Car</a>&emsp;<a data-toggle="modal" id="report" class="btn btn-warning" href="#modal-report">Car Report</a>
           ...
           {{ csrf_field() }}
            ...
            <div class="modal fade" id="modal-report" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content animated fadeIn">
                                        <form target="_blank" action="{{url('report/carhires')}}" method="post">     
                                        <div class="modal-body">
      ...
      @foreach($carhires as $carhire)
        <tr class="{{'del'.$carhire->id}}">
          <td>{{$i}}</td>
          <td><img src="{{url('/public/uploads/logo/'.$carhire->image)}}" width="100" height="100" alt="no logo" /></td>
          <td>{{$carhire->regno}}</td>
          <td>{{$carhire->type}}</td>
          <td>{{$carhire->capacity}}</td>
          <td>{{$carhire->location}}</td>
          <td>{{number_format($carhire->price,2)}}</td>
          <td>
                  ...
                  <ul class="dropdown-menu" role="menu">
                    <li><a class="view" data-toggle="modal" data-regno="{{$carhire->regno}}" data-type="{{$carhire->type}}" data-image="{{$carhire->image}}" data-capacity="{{$carhire->capacity}}" data-location="{{$carhire->location}}" data-price="{{number_format($carhire->price,2)}}" data-id="{{$carhire->id}}" href="#modal-view">View</a></li>

                    <li><a class="edit" data-toggle="modal" data-regno="{{$carhire->regno}}" data-type="{{$carhire->type}}" data-image="{{$carhire->image}}" data-capacity="{{$carhire->capacity}}" data-location="{{$carhire->location}}" data-price="{{number_format($carhire->price,2)}}" data-id="{{$carhire->id}}" href="#modal-form">Update</a></li>
  ...
  $(document).ready(function() {    
    ...
    $('#submit').removeAttr('disabled');
     ...
     function displaydata(){
       $.ajax({
                      url     : "{{URL::to('carhires/showrecord')}}",
                      type    : "GET",
                      async   : false,
   ...
   $('#add').on("click", function() {
    ...
    $("#users").on("click",".view", function(){
     ...
     $(".modal-body .tdprice").html('{{$currency}} '+ price );
   });
